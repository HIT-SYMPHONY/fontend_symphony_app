...
const ManageCompetitions = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError, refetch } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = 1 }) => {
        // Pass the listParams and the current pageNum to the API
        const response = await getAllCompetitions({ ...listParams, pageNum: pageParam })
        return response.data // Return full data object to access meta for pagination
      },
      initialPageParam: 1,
      getNextPageParam: (lastPage) => {
        // Check if there are more pages based on the API's meta data
  ...
  // 3. Trigger fetchNextPage when the user scrolls to the bottom
  useEffect(() => {
  ...
  // Flatten the array of pages into one continuous list
  ...
  return (
    <div>
      ...
      {/* INITIAL LOADING STATE (Matches your exact card layout) */}
      {isLoading &&
      ...
      {!isLoading && competitions.length === 0 && (
        <TextMessage text='Không tìm thấy cuộc thi nào.' />
      )}
        ...
        competitions.map((competition) => {
          return (
            <div key={competition.id} onClick={() => navigate(`${competition.id}`)}>
              <div className='flex gap-4 mb-6 rounded-[20px] p-4 items-center shadow-[0px_0px_4px_2px_#00000033]'>
                ...
                <i className='fa-solid fa-circle-info text-2xl text-primary cursor-pointer hover:opacity-80'></i>
              </div>
            </div>
          )
        })}
