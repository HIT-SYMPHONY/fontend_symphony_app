...
const CreateOfMess = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/admin/competition/${competitionId}/notification`)
    } catch (error) {
      const message = error.response?.data?.message || 'Có lỗi xảy ra khi tạo thông báo.'
      toast.error(message, { id: creationToast })
    } finally {
  ...
  return (
    <div className='mess'>
      <div className='mess__header'>
        <i
          className='mess__back-icon fa-solid fa-arrow-left'
          onClick={() => navigate(`/admin/competition/${competitionId}/notification`)}></i>
        <div
          className='mess__filter'
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          ref={dropdownRef}>
      ...
      <div className='mess__context'>
        ...
        <div className='mess__context-nodung'>
          {/* Your HTML structure does not include a "Tên thông báo" field, which is correct
              since the API only takes "content". I've removed the unused input. */}
          <span>Nội dung</span>
          <textarea
            name='content'
            id='content'
