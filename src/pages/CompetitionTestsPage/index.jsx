  ...
  return (
    <div className='shadow-[0px_0px_4px_2px_#00000040] rounded-[20px] h-full p-4'>
      <h1 className='font-semibold text-2xl mb-4'>
        Phần thi:
        {isLoadingCompetition ? <Skeleton.Input active size='small' /> : competition?.name}
      </h1>
      ...
      <div className='shadow-[0px_0px_4px_2px_#00000033] rounded-[20px] mt-4 flex p-3 items-center gap-2 font-semibold'>
        ...
        <p className='bg-neutral-light text-primary rounded-lg p-1 text-lg mr-auto'>{translateStatus(competition?.status) || 'Chưa cập nhật'}</p>
        <p className='text-primary text-lg mr-8 cursor-pointer' onClick={() => navigate(`/competitions/${competitionId}/test`)}>Làm bài</p>
      </div>
    </div>
  )
}
